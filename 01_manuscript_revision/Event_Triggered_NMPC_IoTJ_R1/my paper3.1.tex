...
\begin{equation}
	...
	\label{eq:jenergy}
\end{equation}
\textcolor{blue}{where $c_1$, $c_2$, and $c_3$ are non-negative weighting coefficients. The first term captures motion-related energy, including horizontal motion, the second accounts for control effort, and the third implements an asymmetric climb penalty for unnecessary positive altitude gains. Thus, $J_{energy}$ is the energy-aware objective used during optimization. It remains a lightweight surrogate for online NMPC rather than a full rotor-level power model.}
