...
detail_rows = cell(2 * num_trials, 15);
trajectory_rows = cell(0, 8);
paired_results = repmat(struct(), num_trials, 2);
detail_row_index = 0;
...
detail_table = cell2table(detail_rows, 'VariableNames', { ...
    ...
    'c3', 'Success', 'Collision', 'TotalClimbMeter', 'EstimatedEnergyWh', ...
    'PathLengthMeter', 'FlightTimeSec', 'MinClearanceMeter', ...
    'TriggerRatePercent', 'ControlEffort', 'Steps'});
...
function row = make_detail_row(trial_index, scenario_seed, mode_name, params, result)
    row = {trial_index, scenario_seed, mode_name, ...
        params.use_asymmetric_energy, params.c3, result.Success, result.Collision, ...
        result.TotalClimbMeter, result.EstimatedEnergyWh, result.PathLengthMeter, ...
        result.FlightTimeSec, result.MinClearanceMeter, ...
        result.TriggerRatePercent, result.ControlEffort, result.Steps};
end
